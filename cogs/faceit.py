...
class Faceit(commands.Cog):
    ...
    ## Command for refreshing team data
    @commands.command()
    ...
    async def upcoming(self, ctx, team):
        ...
        for match in team_data[team_name]["ESEA"]["matches"]:
            if match["status"] == "SCHEDULED":
                ...
                ## Embed field
                embed.add_field(name=f'vs. **{opponent_name}** in ESEA S52 {league_level}',
    ...
    async def today(self, ctx):
        ...
        for team_name, series in df.items():
            ...
            matches = df[team_name].loc['ESEA']


            # Initialise the list for a league level if it does not exist yet
            if matches['league_level'] not in match_dict:
                match_dict[matches['league_level']] = []

            # Append to list in match dict
            match_dict[matches['league_level']].extend(
                [
                    {
                        "name" : f'',
