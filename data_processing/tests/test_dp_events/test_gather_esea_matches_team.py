...
# --- gather_esea_matches_team  tests---
@pytest.mark.asyncio
@pytest.mark.parametrize(
    "team_id, event_id",
    [
...
async def test_gather_esea_matches_team(
    mock_faceit_data_v1, team_id, event_id):
    
    match_list = await dp_events.gather_esea_matches_team(
        team_id=team_id,
        event_id=event_id,
        faceit_data_v1=mock_faceit_data_v1)
    ...
    assert match_list
    assert all(isinstance(match, dict) for match in match_list)
    assert all("match_id" in match for match in match_list)
    
@pytest.mark.asyncio
@pytest.mark.parametrize(
    "team_id, event_id",
    [
        (None, "event1"), # empty team_id
        ("team1", None), # empty event_id
        ("team2", ["event2", None]) , # invalid event_id
        ('team3', []) , # empty event_id list
    ]
)
async def test_gather_esea_matches_team_invalid_inputs(
    mock_faceit_data_v1, team_id, event_id):
    ...
    assert isinstance(match_list, list)
    assert not match_list  # Should return an empty list for invalid inputs
...
async def test_gather_esea_matches_team_invalid_api_responses(
